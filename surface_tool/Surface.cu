...
)
{
	...
	for (int grid_i = blockIdx.x * blockDim.x + threadIdx.x; grid_i < grid_numbers; grid_i = grid_i + gridDim.x * blockDim.x)
	{
		...
		for (int neighbor_grid_i = 0; neighbor_grid_i < 13; neighbor_grid_i = neighbor_grid_i + 1)//ֻ�����԰�ӳ��
		{
			ddx = dx - 1;
			ddy = dy - 1;
			ddz = dz - 1;
			...
			x_ = (grid_3d_serial.int_x + ddx);//13�������е�һ��

			z = (grid_3d_serial.int_z - ddz);
			y = (grid_3d_serial.int_y - ddy);
			x = (grid_3d_serial.int_x - ddx);//�������ĸ��ӶԳƵ���һ��

			x = x - (x & ((x) >> 31));//���x<0��x=0�����x>=0��x=x
			temp_x = (grid_dimension_minus_one.int_x - x);
			x = x + (temp_x & (temp_x >> 31));//���x<grid_dimension.int_x��x=x�����x>=grid_dimension.int_x��x=grid_dimension_minus_one.int_x
...
)
{
	...
	for (int grid_i = blockIdx.x * blockDim.x + threadIdx.x; grid_i < grid_numbers; grid_i = grid_i + gridDim.x * blockDim.x)
	{
		...
		//�ڽ�6�����Ӿ�Ϊ1�����Լ�Ϊ0������Ϊԭʼorigin_grid_occupation
		...
		//�������Χ��temp_occupation=1����Ȼ��Ϊ0
		surface[grid_i] = origin_grid_occupation[grid_i] & (1 - temp_occupation);//ֻ�е�origin_grid_occupation[grid_i]=1��temp_occupation=0��Ϊ1
	}
}
...
void SURFACE::Initial(const float grid_length, const int atom_numbers, const VECTOR* atom_crd, const int* atomic_number)
{
	...
	Smooth_Grid_Occupation_Device << <64, 64 >> >
		(
			...
			);//�����Σ������d_origin_grid_occupationʵ��Ϊ����smoothed�Ľ��
	Build_Surface_Device << <64, 64 >> >
		(
			grid_numbers, layer_numbers, grid_dimension, grid_dimension_minus_one,
			d_origin_grid_occupation, d_smoothed_grid_occupation
			);//��ʱd_smoothed_grid_occupation��¼�����Ǳ��������
